...
function InputComponent({ className, ...props }: ComponentProps<typeof Input>) {
  ...
  return (
    <Input
      className={cn(
        'border-input dark:bg-input/30 rounded-s-none bg-transparent focus:z-1',
        variant === 'sm' && 'h-7',
        variant === 'lg' && 'h-10',
        className
      )}
      ...
      <ComboboxTrigger
        render={
          <Button
            ...
            size={variant}
            className={cn(
              'border-input dark:bg-input/30 dark:hover:bg-input/30 flex gap-1 rounded-s-md rounded-e-none border-e-0 bg-transparent px-2.5 py-0 leading-none shadow-xs hover:bg-transparent focus:z-10 data-pressed:bg-transparent',
              disabled && 'opacity-50'
            )}
            disabled={disabled}
          >
...
interface PhoneDisplayProps {
  ...
  /** Render the number as a clickable tel: link */
  asLink?: boolean;
  className?: string;
...
function PhoneDisplay({
  ...
  const content = (
    <span className={cn('inline-flex items-center gap-1.5', className)}>
      ...
      <span className={cn('text-sm', numberClassName)}>{formatted}</span>
    </span>
  );
