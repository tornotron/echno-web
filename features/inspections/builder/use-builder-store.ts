...
interface BuilderState {
  ...
  setSetting: (key: keyof ChecklistSchema['settings'], value: boolean) => void;
...
/**
 * Input elements that appear *before* `elementId` in render order — the only
 * valid sources for a conditional-visibility rule, since a condition may not
 * depend on an answer the inspector has not reached yet.
 */
export function conditionSourcesFor(
  schema: ChecklistSchema,
  elementId?: string
): ChecklistElement[] {
  const ordered = flattenElements(schema.elements);
  const index = ordered.findIndex((element) => element.id === elementId);
  const candidates = index === -1 ? ordered : ordered.slice(0, index);
  return candidates.filter((element) => isInputElement(element.type));
}
