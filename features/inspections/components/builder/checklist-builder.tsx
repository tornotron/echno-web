...
interface ChecklistBuilderProps {
  ...
  /** Snapshot the schema as a new immutable version. */
...
export function ChecklistBuilder({
  ...
  // `onSave` / `onPublish` may return a promise that rejects. The mutation
  // already reports the failure as a toast, so the rejection is handled — but
  // leaving it unhandled surfaces Next's dev error overlay on top of the
  // builder, which reads as a crash. Swallow it at every call site.
  ...
  const publish = useCallback(() => {
    Promise.resolve(onPublish?.(schema)).catch(() => {});
  }, [onPublish, schema]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') {
        if (paletteOpen || propertiesOpen || confirmDiscard || confirmPublish)
          return;
        requestClose();
      }
    };
  ...
  const conditionSources = useMemo(
    () => conditionSourcesFor(schema, selectedElementId),
      ...
      <AlertDialog open={confirmPublish} onOpenChange={setConfirmPublish}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Publishing saves the current draft and snapshots it as a new
              version. New inspections will pin to that version; inspections
              already created keep the version they were made with, so nothing
              in progress changes underneath an inspector.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
/**
 * Preview keeps its own throwaway response state so an author can exercise
 * conditional logic without those answers ever reaching a submission.
 */
