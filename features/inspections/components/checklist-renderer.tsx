 ...
 *
 * This is the *only* runtime rendering path — the builder's preview mode and
 * the live inspection page both mount this component, so there is no second
 * hand-maintained copy of the checklist UI to drift.
 */
...
interface ChecklistRendererProps {
  ...
  /** Renders every control inert — used by the builder preview. */
  disabled?: boolean;
  /**
   * Optional per-item action rendered beside a field, given its current
   * answer. The runtime uses it to offer "Raise NCR" on a failed item; the
   * builder preview passes nothing, so preview stays a pure render.
   */
  renderItemAction?: (
...
function RenderedField({
  ...
  const action = renderItemAction?.(element, value);
...
function RenderedSection(props: RenderedNodeProps) {
  ...
  // that first shows the errors — no flash of a collapsed section, and no
