...
interface WelcomeScreenProps {
  /** The `?error=` code the page was sent back with, if any. */
  errorCode?: string;
  /** Whether a usable session already exists; only consulted when `errorCode` is set. */
  hasSession: boolean;
}

export function WelcomeScreen({ errorCode, hasSession }: WelcomeScreenProps) {
  return (
    <StarsBackground className="flex min-h-screen items-center justify-center">
      <div className="flex flex-col items-center gap-6 px-4">
        ...
        {errorCode && (
          <AuthErrorNotice code={errorCode} hasSession={hasSession} />
        )}
