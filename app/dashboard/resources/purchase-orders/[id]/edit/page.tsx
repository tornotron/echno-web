...
export default function EditPurchaseOrderPage() {
  ...
  const addLineItem = () => {
    if (!isEditable) {
      toast.error('Cannot add items to approved or sent purchase orders');
      return;
    }
  ...
  const removeLineItem = (id: string) => {
    if (!isEditable) {
      toast.error('Cannot remove items from approved or sent purchase orders');
      return;
    }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                {po.poNumber} - Created on {format(po.poDate, 'MMM dd, yyyy')}
              </p>
            </div>
