...
export default function SubContractEditPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate API call
    setTimeout(() => {
      toast.success(isEditMode ? 'Sub-contract updated successfully' : 'Sub-contract created successfully');
      router.push('/dashboard/third-party/sub-contracts');
    }, 500);
  };

  const handleCancel = () => {
    router.push(isEditMode ? `/dashboard/third-party/sub-contracts/${params.id}` : '/dashboard/third-party/sub-contracts');
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mt-1">
              {isEditMode ? `Update sub-contract information for ${formData.contractorName}` : 'Fill in the details to add a new sub-contract'}
            </p>
          </div>
        </div>
