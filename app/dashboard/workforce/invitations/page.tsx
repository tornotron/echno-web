...
const mockInvitationsExtended = mockInvitations.map((inv, index) => ({
  ...
  createdDate: new Date(new Date(inv.joiningDate || new Date()).getTime() - 15 * 24 * 60 * 60 * 1000),
...
if (mockInvitationsExtended.length > 1) mockInvitationsExtended[1].status = "accepted"
if (mockInvitationsExtended.length > 2) mockInvitationsExtended[2].status = "expired"
...
export default function InvitationsPage() {
  ...
  const pendingInvitations = mockInvitationsExtended.filter((i) => i.status === "pending").length
  const acceptedInvitations = mockInvitationsExtended.filter((i) => i.status === "accepted").length
  const expiredInvitations = mockInvitationsExtended.filter((i) => i.status === "expired").length
              ...
              <TableBody>
                {currentInvitations.length > 0 ? (
                  currentInvitations.map((invitation) => (
                    <TableRow key={invitation.inviteCode} className="border-b border-zinc-200 dark:border-zinc-800">
                      <TableCell>
                        <input type="checkbox" className="rounded" />
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1 text-sm text-zinc-900 dark:text-zinc-100">
                          ...
                          {invitation.expiryDate ? format(invitation.expiryDate, "HH:mm") : "—"}
                        </div>
                      </TableCell>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Link href={`/dashboard/workforce/invitations/${invitation.inviteCode}`}>
                                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100">
